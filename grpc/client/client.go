...
import (
	"context"
	"fmt"
	"github.com/wangxin0814/opentelemetry/grpc/api"
	"github.com/wangxin0814/opentelemetry/grpc/config"
...
func main() {
	tp, err := config.Init("grpc-client")
	if err != nil {
		log.Fatal(err)
	}
	defer func() {
	...
	conn, err = grpc.Dial(":7777", grpc.WithTransportCredentials(insecure.NewCredentials()),
		grpc.WithUnaryInterceptor(otelgrpc.UnaryClientInterceptor()),   // 一元拦截器
		grpc.WithStreamInterceptor(otelgrpc.StreamClientInterceptor()), // 流式拦截器
	)
